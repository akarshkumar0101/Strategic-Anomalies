...
public class PathFinder {
	...
	public static Path getPath(Unit unit, Coordinate moveTo) {
		return PathFinder.getClearPathTo(unit, unit.getPosProp().getCurrentPropertyValue(), moveTo,
				unit.getMovingProp().getCurrentPropertyValue());
	}

	public static Path getClearPathTo(Unit unit, Coordinate from, Coordinate to, int moveRange) {
		return PathFinder.getClearPathTo(unit, from, to, moveRange, null, null);
	}

	public static Path getClearPathTo(Unit unit, Coordinate from, Coordinate to, int remainingMoveRange,
		...
		for (Direction dir : Direction.values()) {
			...
			try {
				...
				Path continuedPath = PathFinder.getClearPathTo(unit, next, to, remainingMoveRange - 1, dir, newPath);
